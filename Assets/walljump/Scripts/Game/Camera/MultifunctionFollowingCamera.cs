...
/// <summary>
/// このカメラ操作スクリプトを使用させていただいた
/// CinemaChine用に変更を加えたり個人的な機能を追加している
/// https://qiita.com/yoship1639/items/1d49d5b481f988da7142
/// </summary>
public class MultifunctionFollowingCamera : MonoBehaviour
{
    ...
    [SerializeField] public float inputSpeedX = 4.0f;
    [SerializeField] public float inputSpeedY = 4.0f;
    [SerializeField] public Vector3 freeLookRotation;
    [SerializeField] public float height = 1.3f;
    [SerializeField] public float distance = 5.0f;
    [SerializeField] public Vector3 rotation = new Vector3(20f, 0, 0);
    [SerializeField, Range(0.01f, 100.0f)] public float positionDamping = 1.0f;
    [SerializeField, Range(0.01f, 100.0f)] public float rotationDamping = 1.0f;
    ...
    void OnEnable()
    {
        StartIventManager.PlayableStartIvent += OnNonInput;
        StartIventManager.PlayableEndIvent += OnInput;
    }
    void OnDisable()
    {
        StartIventManager.PlayableStartIvent -= OnNonInput;
    ...
    void Start()
    {
        ...
        if(enableDollyZoom)
        {
            ...
            cam.m_Lens.FieldOfView = dollyFoV;
        }
    ...
    #region 追加関数
    //わざわざonenableInput = !onenableInput;にしないで関数を分けている
    //切り替えという意味での関数ならそれでいいが確実にこの時はオン　この時はオフト明示するために今回は分けた
    void OnNonInput()
    {
        enableInput = false;
    }
    ...
    {
        rotation = new Vector3(rotation.x, -target.transform.eulerAngles.y - resetRot.y, resetRot.z);
    }
    public void FollowCam(Vector3 from, Vector3 to)
    {
        float changeAngle = Vector3.SignedAngle(from, to, Vector3.up);
        rotation -= Vector3.up * changeAngle;
    }
    #endregion
    ...
    void Simulate(float deltaTime)
    {
        if(enableInput)
        {
            //入力と入力できない時は常にキャラクタの後ろにリセットされるようになっている
            if(pause.padmode)
            {
                rotation.x -= Input.GetAxis(Name.Input.PadY) * inputSpeedY;
                rotation.x = Mathf.Clamp(rotation.x, -60f, 70f);
                rotation.y -= Input.GetAxis(Name.Input.PadX) * inputSpeedX;
            }
            else
            {
                rotation.x -= Input.GetAxis(Name.Input.MouseY) * inputSpeedY;
                rotation.x = Mathf.Clamp(rotation.x, -60f, 70f);
                rotation.y -= Input.GetAxis(Name.Input.MouseX) * inputSpeedX;
            }
    ...
    float GetFrustomHeight(float distance, float fov)
    {
